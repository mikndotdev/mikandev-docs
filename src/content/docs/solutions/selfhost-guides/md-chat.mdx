...
- An example Docker Compose file is available in the [repository](https://github.com/mikndotdev/chat/blob/master/compose.yml), which has all the necessary services configured except for Logto.
- You can use this file to quickly set up a local development environment with MD Chat, Postgres, Redis, and S3-compatible storage.