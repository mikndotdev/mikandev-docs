...
<Aside>
  Use a password generator for the API tokens (or just smash your keyboard and hope for the best),
  and keep them safe!
</Aside>
...
async function handleDelete(request, env) {
  ...
  try {
    ...
    await env.DB.prepare("DELETE FROM urls WHERE slug = ?").bind(slug).run();
